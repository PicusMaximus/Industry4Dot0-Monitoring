...
onMounted(() => {
  activeDevicesStorage = useStorage('active-devices', [], localStorage, { mergeDefaults: true })
...
<template>
  <div class="flex justify-end">
    <ElButton @click="saveActiveDevices" :disabled="!changedOrder" type="success" class="w-60">Speichern</ElButton>
  </div>
  <ElDivider />
  <div class="flex justify-around">
    <div id="devices-container" class="flex flex-col justify-center items-center h-full w-full">
      <h1 class="font-bold">Verfügbare Jobs {{ `(${devices.length})` }}</h1>
      <draggable v-model="devices" tag="ul" :group="{ name: 'devices', pull: 'clone' }" itemKey="device">
    ...
    <div id="active-devices-container" class="flex flex-col justify-start items-center h-full w-full">
      <h1 class="font-bold">Aktive Jobs {{ `(${activeDevices.length})` }}</h1>
      <draggable @change="changedOrder = true" v-model="activeDevices" tag="ul" :group="{ name: 'devices' }"
        itemKey="activeDevice" style="min-height: 100vh;">
        <template #item="{ element: device }">
          <DeviceCardJobs class="w-96" :device="device" :key="device.id" />
        </template>
      </draggable>
